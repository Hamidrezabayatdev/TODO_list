...
export const ShowTasks = ({tasks, taskDelete, editTask}: ShowTasksProps) => {
  ...
  return (
    <div>
      {tasks.map((task, index) => (
        <div className="p-4 w-full mt-4 flex items-center justify-between rounded-2xl border-[1px] border-zinc-400 dark:border-zinc-600">
          <div className="flex items-center w-full gap-4">
            <div className="w-1 h-16 -mr-4 bg-zinc-500 rounded-l-lg rounded-bl-lg"></div>
            <input className="size-6 -mr-1 rounded-2xl" type="checkbox" name="taskDone" id={index.toString()} />
            <div className='w-full'>
              <div className="text-lg font-bold" key={index}>
                <input className="w-full outline-none" value={editingTaskId === task.id ? editingTaskTitle : task.title} onChange={(e) => setEditingTaskTitle(e.target.value)} type="text" readOnly={editingTaskId !== task.id} />
              </div>
              <div className="text-sm opacity-75" key={index}>
                <input className="w-full outline-none" value={editingTaskId === task.id ? editingTaskDescription : task.description} onChange={(e) => setEditingTaskDescription(e.target.value)} type="text" readOnly={editingTaskId !== task.id} />
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            <div className="group p-1 hover:bg-current rounded-md">
              <svg onClick={() => taskDelete(task.id)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6 group-hover:stroke-zinc-50 dark:group-hover:stroke-zinc-700 transition-all">
